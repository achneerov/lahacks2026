...
router.get('/conversations', requireAuth, requireRecruiter, (req, res) => {
  try {
    const userId = req.user.id;
    ...
    console.error('[recruiter/conversations]', e);
    return res.status(500).json({ error: 'server_error' });
  }
});
...
router.get(
  ...
  (req, res) => {
    ...
    try {
      ...
      console.error('[recruiter/conversations/messages GET]', e);
      return res.status(500).json({ error: 'server_error' });
    }
  }
);
