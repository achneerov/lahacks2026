...
export default function RecruiterMessages() {
  ...
  const loadConversations = useCallback(
    async (opts: { silent?: boolean } = {}) => {
      ...
      try {
        const { conversations: list } = await api.recruiterConversations(token, {
  ...
  const loadThread = useCallback(
    async (conversationId: number, opts: { silent?: boolean } = {}) => {
      ...
      try {
        const data = await api.recruiterConversationMessages(
          token,
          conversationId,
        );
        setThread((prev) => {
  ...
  async function handleSend(e: FormEvent) {
    ...
    setSendError(null);
    try {
      const { message } = await api.recruiterSendMessage(
        token,
        selectedId,
        content,
      );
          ...
          <div style={styles.conversationList}>
            {conversationsLoading && conversations.length === 0 ? (
              ...
              <div style={styles.emptyState}>
                No conversations yet. Applicants and agents will appear here
                once they reach out about your jobs.
              </div>
            ) : (
              conversations.map((c) => {
